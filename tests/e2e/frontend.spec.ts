...
test('Session 恢复、知识库页面、Markdown 导入和 Token 管理', async ({ page }) => {
  ...
  await page.route('**/app-api/v1/**', async (route) => {
    ...
    if (path === '/attachments/upload' && request.method() === 'POST') {
      uploadedGif = request.postDataBuffer()?.includes(Buffer.from('GIF89a')) || false
      return json({ id: 'att_gif_e2e', url: '/app-api/v1/attachments/att_gif_e2e' }, 201)
    }
    if (path === '/attachments/att_gif_e2e' && request.method() === 'GET') {
      return route.fulfill({ status: 200, contentType: 'image/gif', body: Buffer.from(gifData, 'base64') })
    }
  ...
  await editor.evaluate((element, gif) => {
    const clipboard = new DataTransfer()
    clipboard.setData('text/html', `<img src="data:image/gif;base64,${gif}">`)
    clipboard.items.add(new File([new Uint8Array([137, 80, 78, 71])], 'static-preview.png', { type: 'image/png' }))
    element.dispatchEvent(new ClipboardEvent('paste', { clipboardData: clipboard, bubbles: true, cancelable: true }))
  }, gifData)
  await expect.poll(() => uploadedGif).toBe(true)
  await expect(editor.locator('img[src$="/attachments/att_gif_e2e"]')).toBeVisible()
